...
from models.model import base_Model, CNNEncoder2D_SLEEP
from models.model_branch import base_Branch_Model
...
parser.add_argument('--selected_dataset', default='WISDM', type=str,
                    help='Dataset of choice: HHAR, dataset2a, sleepEDF,HAR,pFD,WISDM')
...
spect_model = CNNEncoder2D_SLEEP(configs).to(device)

if training_mode == "fine_tune":
    # load saved model of this experiment
    chkpoint = torch.load('./saved_models/ckp_last_{}.pt'.format(configs.name), map_location=device)
...
if training_mode == "train_linear" or "tl" in training_mode:
    chkpoint = torch.load(os.path.join('saved_models', "ckp_last_{}.pt".format(configs.name)), map_location=device)
    pretrained_dict = chkpoint["model_state_dict"]
    model_dict = model.state_dict()
...
spect_model_optimizer = torch.optim.Adam(spect_model.parameters(), lr=configs.lr,
                                         weight_decay=3e-4)
...
# Trainer
Trainer(model, temporal_contr_model, spect_model, model_optimizer, temporal_contr_optimizer, spect_model_optimizer,
        train_dl, valid_dl, test_dl, device,
